...
nids_free_tcp_stream(struct tcp_stream * a_tcp)
{
  ...
  if(a_tcp->stConnInfo.callClose)
  {
    a_tcp->stConnInfo.callClose(a_tcp);
  }
  a_tcp->stConnInfo.callClose = NULL;
  if(a_tcp->stConnInfo.ucData)
  {
    //printf("FreeMem --> Line = %d a_tcp->stConnInfo.ucData = 0x%p \n",__LINE__,a_tcp->stConnInfo.ucData);
    free(a_tcp->stConnInfo.ucData);
    a_tcp->stConnInfo.ucData = NULL;
  }
...
static void
add_new_tcp(struct tcphdr * this_tcphdr, struct ip * this_iphdr,struct ethhdr *pstEthInfo)
{
  ...
  a_tcp->prev_node = 0;
  a_tcp->ulTimeStamp = time(NULL);
  a_tcp->stEthInfo = *pstEthInfo;
  a_tcp->stConnInfo.eMainType = CONN_MAIN_INVALID;
  if(a_tcp->stConnInfo.ucData)
  {
    free(a_tcp->stConnInfo.ucData);
    a_tcp->stConnInfo.ucData = NULL;
...
void
process_tcp(u_char * data, int skblen,struct ethhdr *pstEthInfo)
{
  ...
  if (!nids_params.net_filter(this_iphdr, skblen,this_tcphdr,IPPROTO_TCP))
	  return;
...
//ECN
  ...
  if (!(a_tcp = find_stream(this_tcphdr, this_iphdr, &from_client))) {
	...
	!(this_tcphdr->th_flags & TH_RST))
      add_new_tcp(this_tcphdr, this_iphdr,pstEthInfo);
    return;
  }
  if (from_client) {
    snd = &a_tcp->client;
...
{
    #define TCP_TIME_OUT (260) /*Nat age time is 300s*/
    ...
    if (!tcp_stream_table || !streams_pool)
        return;
   // printf("check tcp stream \n");
    for (i = 0; i < tcp_stream_table_size && tcp_num > 0 ; i++) 
    {
      a_tcp = tcp_stream_table[i];
      while(a_tcp) 
      {
          t_tcp = a_tcp;
          a_tcp = a_tcp->next_node;
          if(t_tcp->ulTimeStamp != 0 && (t_tcp->ulTimeStamp + TCP_TIME_OUT ) < timestatmp)
          {
              for (j = t_tcp->listeners; j; j = j->next) 
              {
                  t_tcp->nids_state = NIDS_EXITING;
              ...
              // if(t_tcp->stConnInfo.ucData)
              // {
              //   printf("FreeMem --> Line = %d a_tcp->stConnInfo.ucData = 0x%p \n",__LINE__,a_tcp->stConnInfo.ucData);
              //   free(t_tcp->stConnInfo.ucData);
              //   t_tcp->stConnInfo.ucData = NULL;
              // }
          }
      }
    }
